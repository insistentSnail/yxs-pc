<template>
  <div class="practice">
    <div class="Breadcrumb">
      ...
      <span>
        {{
          $route.query.questionFrom == 2
            ? "章节练习"
            : $route.query.questionFrom == 1
            ? "模拟真题"
            : "历年真题"
        }}
      </span>
      <span style="color:#333333" v-if="$route.query.courseName">
        > {{ $route.query.courseName }}</span
      >
    </div>
    <div class="content">
      <div class="content-l">
          ...
          >
            ...
            <div class="analysis">
              <div class="analysis-l">
                ...
                <div class="sign" @click="removeError(item)">
                  <!-- <img
                    :src="
                      isSign
                        ? require('@/assets/images/questionBank/signActive.png')
...
export default {
  ...
  methods: {
    removeError(item) {
      removeErrorQuestion({ id: item.wrongId }).then(res => {
        if (res.data) {
          this.$message({
            message: "移除错题成功",
            type: "success"
    ...
    init() {
      errorQuestions({
        questionFrom: this.$route.query.questionFrom,
        courseId: this.$route.query.courseId,
        code: this.$route.query.code,
        size: 30,
        current: 1
      }).then(res => {
        console.log(res, "错题列表"); //题目有分页
        this.total = res.data.total;
        res.data.records = res.data.records.map(item => ({
          ...item.questionVO,
          questionFrom: item.questionFrom,
          wrongId: item.id
        }));
